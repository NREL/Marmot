...
class mplot(object):
    ...
    def total_cap(self):
        ...
        for scenario in Multi_Scenario:
            Installed_Capacity_Collection[scenario] = pd.read_hdf(os.path.join(PLEXOS_Scenarios, scenario,"Processed_HDF5_folder", scenario+ "_formatted.h5"),"generator_Installed_Capacity")
        ...
        Total_Installed_Capacity_Out = Total_Installed_Capacity_Out.T
        
        fig1 = Total_Installed_Capacity_Out.plot.bar(stacked=False, figsize=(9,6), rot=0, 
                             #color=[PLEXOS_color_dict.get(x, '#333333') for x,val in Total_Installed_Capacity_Out.columns], 
                             edgecolor='black', linewidth='0.1')
       
        for x,val in Total_Installed_Capacity_Out.iterrows():
            print(PLEXOS_color_dict.get(x))
