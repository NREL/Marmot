...
regions = data.table(read.csv('mapping_folder/region2zone.csv'))
regions = as.character(unique(pull(regions,Zones)))
...
#Generation
##Dispatch Stacks
```{r stacked-generation, out.width = "100%", fig.align = 'left'}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
plots_subdir = paste0(plots_dir,'Gen_stack/')
for (region in regions){
  img_fn = paste0(plots_subdir,region,'_Stacked_Gen_Facet_Grid.png')
  if (img_fn %in% list.files(plots_subdir, full.names = T)){ print(ggdraw() + 
                                                                     draw_image(img_fn) + 
                                                                     draw_label(region, y = 0.985) +
...
#Outages
```{r outage-stacks, out.width = "100%", fig.align = 'left'}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
plots_subdir = paste0(plots_dir,'Outage_Stack/')
for (region in regions){
  img_fn = paste0(plots_subdir,region,'_Capacity_Out_Stacked_Facet_Grid.png')
  if (img_fn %in% list.files(plots_subdir, full.names = T)){ print(ggdraw() + 
                                                                     draw_image(img_fn) + 
                                                                     draw_label(region, y = 0.985) +
                                                                     theme(plot.margin = unit(c(0.75,0,0.75,0), "cm"))) }
...
##Zonal Interchange
```{r Zonal Interchange, out.width = "100%", fig.align = 'left'}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
plots_subdir = paste0(plots_dir,'Transmission/')
for (region in regions){
