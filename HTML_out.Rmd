...
plot_dev = as.character(inputs[Input == 'Figure_Format']$User_defined_value)

sections = c('_total_generation','_total_generation')
names(sections) = c('Total Generation','Total Generation Difference')

# set up default options for chunks, turn off error or warning messages
knitr::opts_chunk$set(echo=F, comment=NA, warning=F, message=F, include=T,dev='svg', cache = F, fig.width = 20,results = 'asis',out.width = "50%", fig.asp = 0.675)
...
for (section_name in names(sections)){
  ...
  all.plots = list.files(file.path(parent_dir,scenario_dir,'Figures_Output',paste0(agg_level,sections[section_name])),
