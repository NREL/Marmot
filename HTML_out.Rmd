...
pacman::p_load(ggplot2, data.table, maptools, mapproj, RColorBrewer,tidyverse,cowplot,
               scales, stringr, rmarkdown, gdata, zoo, viridis,png,grid,dplyr)

#Had to take out magick and rgeos for Eagle.
...
regions = data.table(read.csv('mapping_folder/mapping_for_html.csv'))
...
for (region in regions){
  ...
  fns = gsub('//','/',list.files(plots_subdir, full.names = T))
  if (img_fn %in% fns){ print(ggdraw() + draw_image(img_fn) + draw_label(region, y = 0.975)) }
}
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
```
