...
###############################################################################
    ...
    def gen_stack(self):
        ...
        def setup_data(zone_input, scenario, Stacked_Gen):
            ...
            Load = pd.read_csv('/Users/mschwarz/OR OSW local/Solutions/future_infrastructures/2036_stateloads.csv',index_col = 'timestamp')
            Load = Load[zone_input]
            Load = Load.groupby(["timestamp"],as_index = True).sum().squeeze()
            Load = Load.rename(scenario)
            Load = pd.DataFrame(Load)
            Load.index = pd.to_datetime(Load.index)
            Load = Load.loc[Stacked_Gen.index]
            ...
            data = {"Stacked_Gen":Stacked_Gen, "Load":Load,"Net_Load":Net_Load}
        ...
        def mkplot(outputs, zone_input, all_scenarios):
            ...
            for scenario in all_scenarios:
                ...
                Stacked_Gen = mfunc.df_process_gen_inputs(Stacked_Gen, self.ordered_gen,zone_input)
                ...
                Data_Table_Out = pd.concat([Load, Stacked_Gen], axis=1, sort=False)
            ...
            plt.ylabel('Generation (GW)',  color='black', rotation='vertical', labelpad=20)
...
###############################################################################
    def gen_diff(self):
        ...
        for zone_input in self.Zones:
            ...
            Total_Gen_Stack_1 = mfunc.df_process_gen_inputs(Total_Gen_Stack_1, self.ordered_gen,zone_input)
           ...
           # Total_Gen_Stack_2 = Total_Gen_Stack_2.xs(zone_input,level=self.AGG_BY)
            Total_Gen_Stack_2 = mfunc.df_process_gen_inputs(Total_Gen_Stack_2, self.ordered_gen,zone_input)
            #Adds in all possible columns from ordered gen to ensure the two dataframes have same column names
            ...
            Gen_Stack_Out = Total_Gen_Stack_2 - Total_Gen_Stack_1
            ...
            Gen_Stack_Out = Gen_Stack_Out / 1000
            ...
            ax.set_title(self.Scenario_Diff[1].replace('_', ' ') + '  -  ' + self.Scenario_Diff[0].replace('_', ' '))
            ax.set_ylabel('Generation Difference (GW)',  color='black', rotation='vertical')
            ax.set_xlabel('Date ' + '(' + self.timezone + ')',  color='black', rotation='horizontal')
