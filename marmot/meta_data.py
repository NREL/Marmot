...
class MetaData:
        ...
        try:
            if self.read_from_formatted_h5:
                ...
                self.h5_data = h5py.File(
                    os.path.join(self.HDF5_folder_in, filename), "r"
                )
        ...
        try:
            try:
                gen_category = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "objects/generator"])
                )
            except KeyError:
                gen_category = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "objects/generators"])
                )
            gen_category.rename(
        ...
        try:
            try:
                region_gen = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/regions_generators"]
                    )
                )
            except KeyError:
                region_gen = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/region_generators"]
                    )
                )
            region_gen.rename(
        ...
        try:
            try:
                zone_gen = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/zones_generators"]
                    )
                )
            except KeyError:
                zone_gen = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/zone_generators"]
                    )
                )
            zone_gen.rename(
        ...
        try:
            ...
            try:
                generator_headstorage = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/generators_headstorage"
                        ]
                    )
                )
                head_tail[0] = 1
            ...
            try:
                generator_headstorage = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/generator_headstorage"
                        ]
                    )
                )
                head_tail[0] = 1
            ...
            try:
                generator_headstorage = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index
                            + "relations/exportinggenerators_headstorage"
                        ]
                    )
                )
                head_tail[0] = 1
            ...
            try:
                generator_tailstorage = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/generators_tailstorage"
                        ]
                    )
                )
                head_tail[1] = 1
            ...
            try:
                generator_tailstorage = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/generator_tailstorage"
                        ]
                    )
                )
                head_tail[1] = 1
            ...
            try:
                generator_tailstorage = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index
                            + "relations/importinggenerators_tailstorage"
                        ]
                    )
                )
                head_tail[1] = 1
        ...
        try:
            try:
                node_region = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/nodes_region"]
                    )
                )
            except KeyError:
                node_region = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "relations/node_region"])
                )
            node_region.rename(
        ...
        try:
            try:
                node_zone = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "relations/nodes_zone"])
                )
            except KeyError:
                node_zone = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "relations/node_zone"])
                )
        ...
        try:
            try:
                generator_node = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/generators_nodes"]
                    )
                )
            except KeyError:
                generator_node = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/generator_nodes"]
                    )
                )
        ...
        try:
            try:
                regions = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "objects/regions"])
                )
            except KeyError:
                regions = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "objects/region"])
                )
    ...
    def zones(self, filename: str) -> pd.DataFrame:
        ...
        try:
            try:
                zones = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "objects/zones"])
                )
            except KeyError:
                zones = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "objects/zone"])
                )
    ...
    def lines(self, filename: str) -> pd.DataFrame:
        ...
        try:
            try:
                lines = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "objects/lines"])
                )
            except KeyError:
                lines = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "objects/line"])
                )
    ...
    def region_regions(self, filename: str) -> pd.DataFrame:
        ...
        if not self._check_if_existing_filename(filename):
            self._read_data(filename)
        try:
            region_regions = pd.DataFrame(
                np.asarray(self.h5_data[self.start_index + "relations/region_regions"])
            )
        ...
        try:
            try:
                region_interregionallines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/region_interregionallines"
                        ]
                    )
                )
            except KeyError:
                region_interregionallines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/region_interregionalline"
                        ]
                    )
                )
        ...
        try:
            try:
                region_intraregionallines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/region_intraregionallines"
                        ]
                    )
                )
            except KeyError:
                try:
                    region_intraregionallines = pd.DataFrame(
                        np.asarray(
                            self.h5_data[
                                self.start_index + "relations/region_intraregionalline"
                            ]
                        )
                    )
                except KeyError:
                        ...
                        [
                            pd.DataFrame(
                                np.asarray(
                                    self.h5_data[
                                        self.start_index
                                        + "relations/region_importinglines"
                                    ]
                                )
                            ),
                            pd.DataFrame(
                                np.asarray(
                                    self.h5_data[
                                        self.start_index
                                        + "relations/region_exportinglines"
                                    ]
                                )
                            ),
                        ]
    ...
    def region_exporting_lines(self, filename: str) -> pd.DataFrame:
        ...
        try:
            try:
                region_exportinglines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/region_exportinglines"
                        ]
                    )
                )
            except KeyError:
                region_exportinglines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/region_exportingline"
                        ]
                    )
                )
    ...
    def region_importing_lines(self, filename: str) -> pd.DataFrame:
        ...
        try:
            try:
                region_importinglines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/region_importinglines"
                        ]
                    )
                )
            except KeyError:
                region_importinglines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/region_importingline"
                        ]
                    )
                )
        ...
        try:
            try:
                zone_interzonallines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/zone_interzonallines"
                        ]
                    )
                )
            except KeyError:
                zone_interzonallines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/zone_interzonalline"]
                    )
                )
            ...
            zone_interzonallines = pd.DataFrame()
            self.logger.warning(
                "Zone Interzonallines data not included in h5plexos results"
            )
        ...
        try:
            try:
                zone_intrazonallines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[
                            self.start_index + "relations/zone_intrazonallines"
                        ]
                    )
                )
            except KeyError:
                zone_intrazonallines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/zone_intrazonalline"]
                    )
                )
        ...
        try:
            try:
                zone_exportinglines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/zone_exportinglines"]
                    )
                )
            except KeyError:
                zone_exportinglines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/zone_exportingline"]
                    )
                )
        ...
        try:
            try:
                zone_importinglines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/zone_importinglines"]
                    )
                )
            except KeyError:
                zone_importinglines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/zone_importingline"]
                    )
                )
    ...
    def interface_lines(self, filename: str) -> pd.DataFrame:
        ...
        try:
            try:
                interface_lines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/interface_lines"]
                    )
                )
            except KeyError:
                interface_lines = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/interfaces_lines"]
                    )
                )
        ...
        try:
            try:
                reserves = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "objects/reserves"])
                )
            except KeyError:
                reserves = pd.DataFrame(
                    np.asarray(self.h5_data[self.start_index + "objects/reserve"])
                )
        ...
        try:
            try:
                reserves_generators = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/reserves_generators"]
                    )
                )
            except KeyError:
                reserves_generators = pd.DataFrame(
                    np.asarray(
                        self.h5_data[self.start_index + "relations/reserve_generators"]
                    )
                )
