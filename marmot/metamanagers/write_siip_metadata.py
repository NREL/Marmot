...
def format_region_intraregionallines(data: dict) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: Formatted metadata
    """
    df = pd.DataFrame.from_dict(data, orient='index', columns=["from_region", "to_region"])
    df = df.loc[df.from_region == df.to_region]["from_region"]
    return df.reset_index().rename(columns={"index": "child", "from_region": "parent"})[["parent", "child"]]
...
def format_region_interregionallines(data: dict) -> pd.DataFrame:
    ...
    return df.reset_index().rename(columns={"index": "child", 0: "parent"})[["parent", "child"]]
