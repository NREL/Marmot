    ...
    def __init__(self, log_directory: str = "logs", log_suffix: str = None):
        ...
        with open("config/marmot_logging_config.yml", "rt") as f:
            ...
            conf["handlers"]["warning_handler"]["filename"] = conf["handlers"][
                "warning_handler"
            ]["filename"].format(log_directory, "plotter", self.log_suffix)
...
class MarmotPlot(SetupLogger):
    ...
    def run_plotter(self):
        ...
        metadata_HDF5_folder_in = os.path.join(
        ...
        try:
            ordered_gen_categories = pd.read_csv(
                os.path.join(
                    self.mapping_folder, mconfig.parser("ordered_gen_categories")
                )
            )
        except FileNotFoundError:
            self.logger.warning(
        ...
        if (
            set(self.gen_names["New"].unique()).issubset(
                ordered_gen_categories["Ordered_Gen"].str.strip().tolist()
            )
        ) == False:
            missing_gen = set(self.gen_names.New.unique()) - (
                set(ordered_gen_categories["Ordered_Gen"].str.strip().tolist())
            )
            self.logger.warning(
        ...
        try:
            PLEXOS_color_dict = pd.read_csv(
                os.path.join(
                    self.mapping_folder, mconfig.parser("color_dictionary_file")
                )
            )
