...
try:
    ...
    print(
        "Attempted import of Marmot as a module from a Git directory. "
    ...
    def __init__(self, log_directory: str = "logs", log_suffix: str = None):
        ...
        with open("config/marmot_logging_config.yml", "rt") as f:
            ...
            conf["handlers"]["warning_handler"]["filename"] = conf["handlers"][
                "warning_handler"
            ]["filename"].format(log_directory, "formatter", self.log_suffix)
...
class ProcessPLEXOS(Process):
    ...
    def get_input_files(self) -> list:
        ...
        for file in files_list:
            self.hdf5_collection[file] = PLEXOSSolution(
    ...
    def df_process_generator(
        ...
        if not self.Region_Mapping.empty:
            region_gen_mapping_idx = pd.MultiIndex.from_frame(
    ...
    def df_process_region(self, df: pd.DataFrame, model_filename: str) -> pd.DataFrame:
        ...
        if not self.Region_Mapping.empty:
            mapping_idx = pd.MultiIndex.from_frame(
    ...
    def df_process_node(self, df: pd.DataFrame, model_filename: str) -> pd.DataFrame:
        ...
        if not self.Region_Mapping.empty:
            region_mapping_idx = pd.MultiIndex.from_frame(
                node_region_meta.merge(
...
class MarmotFormat(SetupLogger):
    ...
    def run_formatter(
        self, sim_model="PLEXOS", plexos_block="ST", append_block_name=False
    ) -> None:
        ...
        input_folder = os.path.join(
            self.Model_Solutions_folder, str(self.Scenario_name)
        )
        output_folder = os.path.join(
            self.Marmot_Solutions_folder, "Processed_HDF5_folder"
        )
        try:
        ...
        process_sim_model = globals()[f"Process{sim_model}"](
            input_folder,
            self.Region_Mapping,
            self.emit_names,
            self.logger,
        ...
        process_properties = self.Plexos_Properties.loc[
            self.Plexos_Properties["collect_data"] == True
        ]
                ...
                if Processed_Data_Out.empty is False:
                    ...
                    while save_attempt <= 3:
                        try:
                            ...
                            MarmotFormat._save_to_h5(
                                Processed_Data_Out,
                                output_file_path,
                                key=(f'{row["group"]}_' f'{row["data_set"]}'),
                            )
        ...
        # ===============================================================================
        if "generator_Curtailment" not in h5py.File(
            output_file_path, "r"
        ) or not mconfig.parser("skip_existing_properties"):
            try:
                self.logger.info("Processing generator Curtailment")
                try:
                    Avail_Gen_Out = pd.read_hdf(
                        output_file_path, "generator_Available_Capacity"
                    )
                    Total_Gen_Out = pd.read_hdf(
                        output_file_path, "generator_Generation"
                    )
                ...
                MarmotFormat._save_to_h5(
                    Curtailment_Out, output_file_path, key="generator_Curtailment"
                )

                MarmotFormat._save_to_h5(
                    Upward_Available_Capacity,
                    output_file_path,
                ...
                self.logger.warning(
                    "NOTE!! Curtailment not calculated, " "processing skipped\n"
                )

        if "region_Cost_Unserved_Energy" not in h5py.File(
            output_file_path, "r"
        ) or not mconfig.parser("skip_existing_properties"):
            try:
                self.logger.info("Calculating Cost Unserved Energy: Regions")
                Cost_Unserved_Energy = pd.read_hdf(
                    output_file_path, "region_Unserved_Energy"
                )
                ...
                self.logger.warning(
                    "NOTE!! Regional Unserved Energy not available "
                    "to process, processing skipped\n"
                )
                pass

        if "zone_Cost_Unserved_Energy" not in h5py.File(
            output_file_path, "r"
        ) or not mconfig.parser("skip_existing_properties"):
            try:
                self.logger.info("Calculating Cost Unserved Energy: Zones")
                Cost_Unserved_Energy = pd.read_hdf(
                    output_file_path, "zone_Unserved_Energy"
                )
                Cost_Unserved_Energy = Cost_Unserved_Energy * self.VoLL
                ...
                self.logger.warning(
                    "NOTE!! Zonal Unserved Energy not available to "
...
def main():
    ...
    Marmot_user_defined_inputs = pd.read_csv(
        mconfig.parser("user_defined_inputs_file"),
