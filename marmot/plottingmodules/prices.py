...
class MPlot(PlotDataHelper):
    ...
    def pdc_all_regions(
        ...
        **_,
        ...
        fig.savefig(
            os.path.join(save_figures, "Price_Duration_Curve_All_Regions.svg"),
            dpi=600,
            bbox_inches="tight",
        )
        Data_Table_Out.to_csv(
            os.path.join(save_figures, "Price_Duration_Curve_All_Regions.csv")
        )
    ...
    def timeseries_price_all_regions(
        ...
        **_,
        ...
        fig.savefig(
            os.path.join(save_figures, "Price_Timeseries_All_Regions.svg"),
            dpi=600,
            bbox_inches="tight",
        )
        Data_Table_Out.to_csv(
            os.path.join(save_figures, "Price_Timeseries_All_Regions.csv")
        )
        outputs = DataSavedInModule()
        return outputs
        ...
        **_,
        ...
        for scenario in self.Scenarios:
            ...
            if pd.notna(start_date_range):
                ...
                price = price[
                    pd.to_datetime(start_date_range) : pd.to_datetime(end_date_range)
                ]
        ...
        fig.savefig(
            os.path.join(node_figure_folder, figure_name + ".svg"),
            dpi=600,
            bbox_inches="tight",
        )
        ...
        for node in select_nodes:
            ...
            for scenario in self.Scenarios:
                ...
                if pd.notna(start_date_range):
                    ...
                    price = price[
                        pd.to_datetime(start_date_range) : pd.to_datetime(
                            end_date_range
                        )
                    ]
            ...
            if not all_prices:
                self.logger.info(
                    f"Nodes not found in database, input sheet error likely!"
                )
                return InputSheetError()
            ...
            fig.savefig(
                os.path.join(node_figure_folder, f"{figure_name}_{node}.svg"),
                dpi=600,
                bbox_inches="tight",
            )
            data_out.to_csv(
                os.path.join(node_figure_folder, f"{figure_name}_{node}.csv")
            )
