    ...
    def gen_stack(self, figure_name: str = None, prop: str = None,
        ...
        for zone_input in self.Zones:
            ...
            for i, scenario in enumerate(self.Scenarios):
                ...
                if not stacked_curt_df.empty:
                    ...
                    if zone_input in stacked_curt_df.index.get_level_values(self.AGG_BY).unique():
                        ...
                        if self.curtailment_prop == 'Curtailment':
                            stacked_curt_df = self.assign_curtailment_techs(stacked_curt_df)
                        stacked_curt_df = stacked_curt_df.sum(axis=1)
                        stacked_curt_df[stacked_curt_df<0.05] = 0 #Remove values less than 0.05 MW
                        #Insert curtailment into
                        stacked_gen_df.insert(len(stacked_gen_df.columns), 
                                        column=curtailment_name, value=stacked_curt_df)
                ...
                vre_gen_cat = [name for name in vre_gen_cat if name in stacked_gen_df.columns]
                ...
                extra_data_frames = []
                extra_property_names = [f"{agg}_Load{data_resolution}", 
                ...
                for ext_prop in extra_property_names:
                    ...
                    if df.empty or not plot_data_settings["include_stackplot_load_lines"]:
                ...
                if (extra_plot_data[f"{agg}_Unserved_Energy{data_resolution}"] == 0).all() == False:
                ...
                extra_plot_data = extra_plot_data.loc[stacked_gen_df.index.min(): 
                                                        stacked_gen_df.index.max()]
                # append net_load load to extra data, 
                # fill na with 0 in the event of misaligned timeseries 
                extra_plot_data = pd.concat([extra_plot_data, net_load], axis=1).fillna(0)

                # unitconversion based off peak generation hour, only checked once
                if i == 0:
                    unitconversion = self.capacity_energy_unitconversion(stacked_gen_df, 
                                                                        sum_values=True)
                #Convert units
                ...
                if pd.notna(prop):
                    x_time_value = mplt.add_property_annotation(
                                            pd.concat([stacked_gen_df, 
                                                       extra_plot_data], axis=1), 
                                            prop,
                                            sub_pos=i,
                ...
                stacked_gen_df = stacked_gen_df.loc[:, (stacked_gen_df != 0).any(axis=0)]

                # Data table of values to return to main program
                single_scen_out = pd.concat([extra_plot_data, stacked_gen_df], 
                                            axis=1, sort=False)
                ...
                single_scen_out = single_scen_out.loc[:, (single_scen_out != 0).any(axis=0)]
                data_tables.append(single_scen_out)

                mplt.stackplot(stacked_gen_df, self.PLEXOS_color_dict, 
                                      labels=stacked_gen_df.columns, sub_pos=i)
                mplt.set_subplot_timeseries_format(sub_pos=i)

                if (extra_plot_data['Unserved Energy'] == 0).all() == False:
                    axs[i].fill_between(extra_plot_data["Total Demand"].index, 
                ...
                if plot_data_settings["include_stackplot_load_lines"]:
                    if plot_data_settings["include_timeseries_load_storage_charging_line"] and \
