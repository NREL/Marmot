        ...
        **_,
        ...
        for zone_input in self.Zones:
            ...
            for scenario in self.Scenarios:
                ...
                Total_Systems_Cost.columns = Total_Systems_Cost.columns.str.replace(
                    "_", " "
                )
            ...
            Total_Systems_Cost_Out.index = Total_Systems_Cost_Out.index.str.replace(
                "_", " "
            )
            ...
            ax.set_ylabel(
                "Total System Net Rev, Rev, & Cost ($/KW-yr)",
                color="black",
                rotation="vertical",
            )
            ax.margins(x=0.01)
        ...
        **_,
        ...
        for zone_input in self.Zones:
            ...
            for scenario in self.Scenarios:
                ...
                Total_Systems_Cost = pd.concat(
                    [Total_Systems_Cost, Total_Gen_Cost, Cost_Unserved_Energy],
                    axis=1,
                    sort=False,
                )
        ...
        **_,
        ...
        for zone_input in self.Zones:
            ...
            for scenario in self.Scenarios:
                ...
                Start_Shutdown_Cost = self["generator_Start_&_Shutdown_Cost"].get(
                    scenario
                )
                Start_Shutdown_Cost = Start_Shutdown_Cost.xs(
                    zone_input, level=self.AGG_BY
                )
                ...
                if Emissions_Cost.empty:
                    self.logger.warning(
                        f"generator_Emissions_Cost not included in {scenario} results, Emissions_Cost will not be included in plot"
                    )
                    Emissions_Cost = self["generator_Start_&_Shutdown_Cost"][
                        scenario
                    ].copy()
                ...
                Detailed_Gen_Cost = pd.concat(
                    [Fuel_Cost, VOM_Cost, Start_Shutdown_Cost, Emissions_Cost],
                    axis=1,
                    sort=False,
                )

                Detailed_Gen_Cost.columns = Detailed_Gen_Cost.columns.str.replace(
                    "_", " "
                )
            ...
            Detailed_Gen_Cost_Out = (
                Detailed_Gen_Cost_Out.T / 1000000
            )  # Convert cost to millions

            Detailed_Gen_Cost_Out.index = Detailed_Gen_Cost_Out.index.str.replace(
                "_", " "
            )
        ...
        **_,
        ...
        for zone_input in self.Zones:
            ...
            Total_Generation_Stack_Out = pd.concat(
                gen_cost_out_chunks, axis=1, sort=False
            ).fillna(0)
            Total_Generation_Stack_Out = self.create_categorical_tech_index(
                Total_Generation_Stack_Out
            )
            Total_Generation_Stack_Out = (
                Total_Generation_Stack_Out.T / 1000000
            )  # Convert to millions
            ...
            if pd.notna(custom_data_file_path):
                Total_Generation_Stack_Out = self.insert_custom_data_columns(
                    Total_Generation_Stack_Out, custom_data_file_path
                )
            # Data table of values to return to main program
    ...
    def sys_cost_diff(
        ...
        for zone_input in self.Zones:
            ...
            try:
                Total_Systems_Cost_Out = (
                    Total_Systems_Cost_Out
                    - Total_Systems_Cost_Out.xs(self.Scenarios[0])
                )  # Change to a diff on first scenario
            except KeyError:
                outputs[zone_input] = MissingZoneData()
                continue
            Total_Systems_Cost_Out.drop(
            ...
            ax.set_ylabel(
                "Generation Cost Change (Million $) \n relative to "
                + self.Scenarios[0],
                color="black",
                rotation="vertical",
            )
    ...
    def sys_cost_type_diff(
        ...
        for zone_input in self.Zones:
            ...
            try:
                Total_Generation_Stack_Out = (
                    Total_Generation_Stack_Out
                    - Total_Generation_Stack_Out.xs(self.Scenarios[0])
                )  # Change to a diff on first scenario
            except KeyError:
                outputs[zone_input] = MissingZoneData()
                continue
            Total_Generation_Stack_Out.drop(
                self.Scenarios[0], inplace=True
            )  # Drop base entry
            ...
            mplt.barplot(
                Total_Generation_Stack_Out, color=self.PLEXOS_color_dict, stacked=True
            )
    ...
    def detailed_gen_cost_diff(
        ...
        for zone_input in self.Zones:
            ...
            for scenario in self.Scenarios:
                ...
                if Emissions_Cost.empty:
                    self.logger.warning(
                        f"generator_Emissions_Cost not included in {scenario} results, "
            ...
            try:
                Detailed_Gen_Cost_Out = (
                    Detailed_Gen_Cost_Out - Detailed_Gen_Cost_Out.xs(self.Scenarios[0])
                )  # Change to a diff on first scenario
