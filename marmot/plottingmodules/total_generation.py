...
class MPlot(PlotDataHelper):
    ...
    def total_gen(self, start_date_range: str = None, end_date_range: str = None, **_):
        ...
        for zone_input in self.Zones:
            ...
            for scenario in self.Scenarios:
                ...
                Unserved_Energy = Unserved_Energy.rename(columns={0: scenario}).sum(
                    axis=0
                )
                ...
                if Pump_Load.empty or not mconfig.parser(
                    "plot_data", "include_total_pumped_load_line"
                ):
                ...
                if self.shift_leapday == True:
                    batt_load = self.adjust_for_leapday(batt_load)
            ...
            Total_Load_Out = Total_Load_Out.rename(
                "Total Load (Demand + \n Storage Charging)"
            )
            ...
            if mconfig.parser("plot_data", "include_total_net_imports"):
                Total_Generation_Stack_Out = self.include_net_imports(
                    Total_Generation_Stack_Out, Total_Load_Out, Unserved_Energy_Out
                )
            ...
            for n, scenario in enumerate(self.Scenarios):
                ...
                if Pump_Load_Out[scenario] > 0:
                    ...
                    ax.plot(x, height2, "r--", c="black", linewidth=1.5, label="Demand")
                else:
                    ax.plot(x, height1, c="black", linewidth=3, label="Demand")
                ...
                if Unserved_Energy_Out[scenario] > 0:
                    ...
                    ax.fill_between(x, height3, height1, facecolor="#DD0200", alpha=0.5)
            # Add legend
    ...
    def _monthly_gen(self, vre_only: bool = False, plot_as_percnt: bool = False, **_):
        ...
        for zone_input in self.Zones:
            ...
            for i, scenario in enumerate(self.Scenarios):
                ...
                def _group_monthly_data(input_df, interval_count):
                    ...
                    monthly_df["timestamp"] = monthly_df["timestamp"].dt.month.apply(
                        lambda x: self.MONTHS[x]
                    )
                ...
                monthly_total_load = monthly_total_load.set_index(
                    [scenario_names], append=True
                )
                monthly_pumped_load = monthly_pumped_load.set_index(
                    [scenario_names], append=True
                )
                monthly_total_demand = monthly_total_demand.set_index(
                    [scenario_names], append=True
                )
            ...
            Total_Load_Out.rename(
                columns={0: "Total Load (Demand + \n Storage Charging)"}, inplace=True
            )

            # Add Net Imports if desired
            if (
                mconfig.parser("plot_data", "include_total_net_imports")
                and not vre_only
            ):
            ...
            if not plot_as_percnt:
                ...
                Data_Table_Out = pd.concat(
                    [Total_Load_Out, Total_Demand_Out, Pump_Load_Out, Gen_Out], axis=1
                ...
                if not vre_only:
                    for n, _m in enumerate(month_total_load.index):
                        ...
                        if month_pumped_load.loc[_m].values.sum() > 0:
                            ...
                            axs[i].plot(
                                x, height1, c="black", linewidth=3, label="Demand"
                            )
