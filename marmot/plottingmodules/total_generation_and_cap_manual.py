...
Total_Gen_Stack.insert(
    len(Total_Gen_Stack.columns), column="Curtailment", value=Stacked_Curt
)
...
if (
    Unserved_Energy.empty
):  # If unserved energy doesn't exist, create a copy of the load dataframe, with all values = 0.
...
if (
    Pump_Load.empty
):  # If pump load doesn't exist, create a copy of the load dataframe, with all values = 0.
...
Total_Installed_Capacity = pd.read_csv(
    "/path/to/capacity_file.csv", index_col="regions"
)

# Color dictionary (Marty to provide mapping file).
PLEXOS_color_dict = pd.read_csv("/path/to/colormapping_file.csv")
PLEXOS_color_dict = PLEXOS_color_dict.rename(
    columns={
        PLEXOS_color_dict.columns[0]: "Generator",
        PLEXOS_color_dict.columns[1]: "Colour",
    }
)
...
Data_Table_Out = pd.concat(
    [Total_Load.T, Total_Demand.T, Unserved_Energy.T, Total_Gen_Stack],
    axis=1,
    sort=False,
)
...
Total_Installed_Capacity.plot.bar(
    stacked=True,
    ax=axs[0],
    color=[
        PLEXOS_color_dict.get(x, "#333333") for x in Total_Installed_Capacity.columns
    ],
    edgecolor="black",
    linewidth="0.1",
)
axs[0].set_ylabel(
    f"Total Installed Capacity ({capacity_units})", color="black", rotation="vertical"
)
...
Total_Gen_Stack.plot.bar(
    stacked=True,
    ax=axs[1],
    color=[PLEXOS_color_dict.get(x, "#333333") for x in Total_Gen_Stack.columns],
    edgecolor="black",
    linewidth="0.1",
)

axs[1].set_ylabel(
    f"Total Generation ({unitconversion['units']}h)", color="black", rotation="vertical"
)
axs.yaxis.set_major_formatter(
    mpl.ticker.FuncFormatter(lambda x, p: format(x, f",.{y_axes_decimalpt}f"))
)
...
if Unserved_Energy.values.sum() > 0:
    custom_legend_elements = Patch(
        facecolor="#DD0200", alpha=0.5, edgecolor="#DD0200", label="Unserved Energy"
    )
...
axs[0].set_title(
    "set left panel title here",
    fontdict={"weight": "bold", "size": 11},
    loc="left",
    pad=4,
)
axs[1].set_title(
    "set right panel title here",
    fontdict={"weight": "bold", "size": 11},
    loc="left",
    pad=4,
)
