        ...
        for region in self.Zones:
            ...
            for n, scenario in enumerate(Scenarios):
                ...
                if prop == "Peak Demand":
                    ...
                    total_reserve = (
                        reserve_provision_timeseries.sum(axis=1)
                    ...
                    reserve_provision_timeseries = reserve_provision_timeseries[
                        start_date:end_date
                    ]
                ...
                if prop == "Peak Demand":
                    axs[n].annotate(
                        "Peak Reserve: \n"
                        + str(format(int(Peak_Reserve), ".2f"))
                        + " {}".format(unitconversion["units"]),
                        xy=(peak_reserve_t, Peak_Reserve),
                        xytext=(
                            (peak_reserve_t + dt.timedelta(days=0.25)),
                            (Peak_Reserve + Peak_Reserve * 0.05),
                        ),
                        fontsize=13,
                        arrowprops=dict(facecolor="black", width=3, shrink=0.1),
                    )
    ...
    def total_reserves_by_gen(
        ...
        for region in self.Zones:
            ...
            for scenario in self.Scenarios:
                ...
                Total_Reserves_Out = pd.concat(
                    [Total_Reserves_Out, reserve_provision], axis=1, sort=False
                ).fillna(0)
            ...
            Total_Reserves_Out.index = Total_Reserves_Out.index.str.wrap(
                5, break_long_words=False
            )
        ...
        for region in self.Zones:
            ...
            for scenario in self.Scenarios:
                ...
                if count_hours == False:
                    reserve_total = (
                        reserve_timeseries.groupby(["Type"]).sum() / interval_count
                    )
                    ...
                    reserve_total = (
                        reserve_total.groupby("Type").count() / interval_count
                    )
        ...
        for region in self.Zones:
            ...
            for n, scenario in enumerate(Scenarios):
                ...
                if pd.notna(start_date_range):
                    ...
                    reserve_timeseries = reserve_timeseries[
                        start_date_range:end_date_range
                    ]
                else:
                    self.logger.info("Plotting graph for entire timeperiod")
