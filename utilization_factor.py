...
#            Total_Ava.rename(scenario,inplace=True)
            ...
            for i in Gen.reset_index()['tech'].unique():
    ...
    def uf_gen(self):
        ...
        for scenario in self.Multi_Scenario:
            ...
            Gen['Hour CF']= Gen['0_x']/Gen['0_y']                                                       #Hourly CF individual generators
            ...
            thermal_generator_cf=thermal_generator_cf[(thermal_generator_cf['Hour CF'].isna())==False]  #Remove na's for categories that don't match gens Add check that the same number of entries is found?
            
            m=0
            for i in thermal_generator_cf.reset_index()['tech'].unique():
                cfs = thermal_generator_cf['Hour CF'].xs(i,level="tech")
