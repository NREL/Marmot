...
class mplot(object):
    ...
    def __init__(self,argument_list):
        ...
        self.facet_gen_cat = argument_list[25]

    # def gen_stack_OLD(self):
    #
    ...
    def gen_stack(self):
        ...
        for scenario in self.Multi_Scenario:
            ...
            if '2008' not in self.Marmot_Solutions_folder and '2012' not in self.Marmot_Solutions_folder and Stacked_Gen.index[0] > dt.datetime(2024,2,28,0,0):
              ...
              if (Pump_Load == 0).all() == False:    #When Load == Total Demand, the load line gets shifted by two days. This fixes that bug.
                  Total_Demand.index = Total_Demand.index.shift(1,freq = 'D')

            Data_Table_Out = pd.concat([Load, Total_Demand, Unserved_Energy, Stacked_Gen], axis=1, sort=False)
            ...
            if len(self.Multi_Scenario) > 1:
                ...
                #Legend 2
                if (Pump_Load == 0).all() == False:
                    leg2 = axs[grid_size-1].legend(lp, ['Demand + Storage Charging'], loc='upper left',bbox_to_anchor=(1, 1.55),
                              facecolor='inherit', frameon=True)
                else:
                ...
                sp = axs.stackplot(Stacked_Gen.index.values, Stacked_Gen.values.T, labels=Stacked_Gen.columns, linewidth=0,
                             colors=[self.PLEXOS_color_dict.get(x, '#333333') for x in Stacked_Gen.T.index])
                if (Unserved_Energy == 0).all() == False:
                    lp2 = axs.plot(Unserved_Energy,
                                      #color='#EE1289'  OLD MARMOT COLOR
                                      color = '#DD0200' #SEAC STANDARD COLOR (AS OF MARCH 9, 2020)
                                      )
                lp = axs.plot(Load, color='black')
                if (Pump_Load == 0).all() == False:
                    lp3 = axs.plot(Total_Demand, color='black', linestyle="--")
                ...
                if self.prop == "Min Net Load":
                    axs.annotate('Min Net Load: \n' + str(format(int(Min_Net_Load), ',')) + ' MW', xy=(min_net_load_t, Min_Net_Load), xytext=((min_net_load_t + dt.timedelta(days=0.1)), (Min_Net_Load + max(Load)/4)),
                        fontsize=13, arrowprops=dict(facecolor='black', width=3, shrink=0.1))

                elif self.prop == "Peak Demand":
                    axs.annotate('Peak Demand: \n' + str(format(int(Total_Demand[peak_demand_t]), ',')) + ' MW', xy=(peak_demand_t, Peak_Demand), xytext=((peak_demand_t + dt.timedelta(days=0.1)), (max(Total_Demand) + Total_Demand[peak_demand_t]*0.1)),
                                fontsize=13, arrowprops=dict(facecolor='black', width=3, shrink=0.1))


                if (Unserved_Energy == 0).all() == False:
                    axs.fill_between(Load.index, Load,Unserved_Energy,
                                       # facecolor='#EE1289' OLD MARMOT COLOR
                                        facecolor = '#DD0200', #SEAC STANDARD COLOR (AS OF MARCH 9, 2020)
                                        alpha=0.5)
                ...
                #Legend 2
                if (Pump_Load == 0).all() == False:
                    leg2 = axs.legend(lp, ['Demand + Storage Charging'], loc='upper left',bbox_to_anchor=(1, 1.55),
                              facecolor='inherit', frameon=True)
                else:
                    leg2 = axs.legend(lp, ['Demand'], loc='upper left',bbox_to_anchor=(1, 1.55),
                              facecolor='inherit', frameon=True)

                #Legend 3
                if (Unserved_Energy == 0).all() == False:
                    leg3 = axs.legend(handles=custom_legend_elements, loc='upper left',bbox_to_anchor=(1, 1.35),
                          facecolor='inherit', frameon=True)

                #Legend 4
                if (Pump_Load == 0).all() == False:
                    leg4 = axs.legend(lp3, ['Demand'], loc='upper left',bbox_to_anchor=(1, 1.45),
                              facecolor='inherit', frameon=True)
    ...
    def gen_stack_all_periods(self):
       ...
       # try:   #The rest of the function won't work if this particular zone can't be found in the solution file (e.g. if it doesn't include Mexico)
        ...
        for wk in range(1,53): #assumes weekly, could be something else if user changes self.end Marmot_plot_select
            ...
            fig1, ax = plt.subplots(figsize=(9,6))
            sp = ax.stackplot(Stacked_Gen_Period.index.values, Stacked_Gen_Period.values.T, labels=Stacked_Gen_Period.columns, linewidth=5,colors=[self.PLEXOS_color_dict.get(x, '#333333') for x in Stacked_Gen_Period.T.index])
            
            if (Unserved_Energy_Period == 0).all() == False:
                lp2 = plt.plot(Unserved_Energy_Period, 
                               #color='#EE1289'  OLD MARMOT COLOR
                               color = '#DD0200' #SEAC STANDARD COLOR (AS OF MARCH 9, 2020)
                               )
            ...
            fig1.savefig(os.path.join(gen_stack_figures, self.zone_input + "_" + "Stacked_Gen_All_Periods" + "_" + self.Multi_Scenario[0]+"_period_"+str(wk)), dpi=600, bbox_inches='tight')
            Data_Table_Out.to_csv(os.path.join(gen_stack_figures, self.zone_input + "_" + "Stacked_Gen_All_Periods" + "_" + self.Multi_Scenario[0]+"_period_"+str(wk)+ ".csv"))
            del fig1
            del Data_Table_Out
    ...
    def committed_stack(self):
            ...
            fig4, axs = plt.subplots(ydimension,xdimension, figsize=((8*xdimension),(4*ydimension)), sharey = False, sharex = True)
            ...
            for scenario in self.Multi_Scenario:
                ...
                thermal_commit_cap = thermal_commit_cap.xs(self.zone_input,level = self.AGG_BY)
