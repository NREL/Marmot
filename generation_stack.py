...
class mplot(object):
    ...
    def outage_stack(self):
        ...
        Forced_Outage_read = pd.read_hdf(hdf_out_folder + "/" + Multi_Scenario[0]+"_formatted.h5", 'generator_Forced_Outage')
       
        # If data is to be aggregated by zone, then zone properties are loaded, else region properties are loaded
        if AGG_BY == "zone":
            Load_read = pd.read_hdf(hdf_out_folder + "/" + Multi_Scenario[0]+"_formatted.h5", "zone_Load")
       ...
       # try:   #The rest of the function won't work if this particular zone can't be found in the solution file (e.g. if it doesn't include Mexico)
        Forced_Outage = Forced_Outage_read.xs(zone_input,level=AGG_BY)  
        Forced_Outage = df_process_gen_inputs(Forced_Outage)
 
        # Removes columns that only contain 0
        Forced_Outage = Forced_Outage.loc[:, (Forced_Outage != 0).any(axis=0)]
